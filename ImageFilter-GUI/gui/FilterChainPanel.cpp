    ...
    , ui_(new Ui::FilterChainPanel())
{
    ...
    filterListLayout_ = qobject_cast<QVBoxLayout*>(ui_->filterListWidget->layout());
    if (filterListLayout_ == nullptr)
    {
        filterListLayout_ = new QVBoxLayout(ui_->filterListWidget);
        filterListLayout_->setContentsMargins(0, 0, 0, 0);
        filterListLayout_->setSpacing(5);
    }
    filterListLayout_->addStretch(); // Добавляем растягивающий элемент в конец

    // Подключаем кнопку добавления фильтра
    connect(ui_->addFilterButton, &QPushButton::clicked,
            this, &FilterChainPanel::onAddFilter);

    // Подключаемся к сигналам модели
    connect(filterChainModel_, &FilterChainModel::chainChanged,
            this, &FilterChainPanel::onChainChanged);
    connect(filterChainModel_, &FilterChainModel::filterParameterChanged,
            this, &FilterChainPanel::onChainChanged);
...
{
    if (filterChainModel_ == nullptr || filterListLayout_ == nullptr)
    {
        return;
    }
    ...
    for (size_t i = 0; i < filterCount; ++i)
    {
        ...
        auto* filterWidget = new FilterItemWidget(
            i,
            filter.filterName,
            filter.parameters,
            canRemove,
            ...
            canMoveDown,
            filterChainModel_,
            ui_->filterListWidget);

        // Подключаем сигналы
        connect(filterWidget, &FilterItemWidget::parameterChanged,
                this, [this](size_t index, const std::string& paramName, const QVariant& value) {
                    if (filterChainModel_ != nullptr)
                    {
        ...
        connect(filterWidget, &FilterItemWidget::removeRequested,
                this, [this](size_t index) {
                    if (filterChainModel_ != nullptr)
                    {
                        ...
                        {
                            QMessageBox::warning(this, "Ошибка", 
                                "Не удалось удалить фильтр. Возможно, индекс невалиден.");
                        }
                    }
                });

        connect(filterWidget, &FilterItemWidget::moveUpRequested,
                this, [this](size_t index) {
                    if (filterChainModel_ != nullptr)
                    {
        ...
        connect(filterWidget, &FilterItemWidget::moveDownRequested,
                this, [this](size_t index) {
                    if (filterChainModel_ != nullptr)
                    {
        ...
        connect(filterWidget, &FilterItemWidget::filterTypeChangeRequested,
                this, [this](size_t index) {
                    if (filterChainModel_ != nullptr)
                    {
        ...
        filterListLayout_->insertWidget(static_cast<int>(i), filterWidget);
    }
}
...
void FilterChainPanel::onAddFilter()
{
    ...
    if (!selectedFilter.isEmpty() && filterChainModel_ != nullptr)
    {
        ...
        {
            QMessageBox::warning(this, "Ошибка", 
                "Не удалось добавить фильтр. Возможно, имя фильтра невалидно.");
        }
    }
}
