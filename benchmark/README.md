...
Этот подпроект содержит инструменты для измерения производительности наложения фильтров на изображения. Бенчмарк автоматически тестирует все 22 фильтра на различных размерах изображений и предоставляет детальную статистику производительности.
...
# Тестировать цепочки фильтров (включено по умолчанию)
poetry run benchmark --test-chains

# Исключить тестирование цепочек фильтров
poetry run benchmark --no-chains
```
...
Бенчмарк автоматически тестирует все 22 фильтра, разделенных на категории. Также поддерживается тестирование цепочек фильтров для оценки производительности комбинированных операций.
...
| Фильтр | Изображение | Ширина | Высота | Пикселей | Время (с) | Успех | Ошибка | Цепочка | Длина цепочки |
|--------|-------------|--------|--------|----------|-----------|-------|--------|---------|---------------|
| grayscale | small_gradient_256x256.jpg | 256 | 256 | 65536 | 0.001234 | Да | | Нет | 1 |
| grayscale,sharpen | small_gradient_256x256.jpg | 256 | 256 | 65536 | 0.002456 | Да | | Да | 2 |

Поле "Цепочка" указывает, является ли запись результатом применения цепочки фильтров. Поле "Длина цепочки" показывает количество фильтров в цепочке (1 для одиночных фильтров).
...
### Надежность
- Каждый тест имеет таймаут 5 минут для предотвращения зависаний
