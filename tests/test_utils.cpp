...
namespace TestUtils
{
    ...
    )
    {
        ...
        if (!processor.resize(width, height, channels, allocated_data))
        {
        ...
        if (!processor.saveToFile(filepath, has_alpha))
        {
            return "";
        }
...
TEST_F(BatchProcessorTest, ProcessAll)
{
    ...
    auto process_func = [&processed_count](const std::string& input, const std::string& output) -> bool
    {
        ImageProcessor img;
...
TEST_F(BatchProcessorTest, ProcessAllWithProgress)
{
    ...
    auto progress_callback = [&callback_count](size_t current, size_t total, const std::string&)
    {
        callback_count++;
        EXPECT_GT(current, 0);
    ...
    auto process_func = [](const std::string& input, const std::string& output) -> bool
    {
        ImageProcessor img;
        if (!img.loadFromFile(input))
        {
...
TEST_F(BatchProcessorTest, ProcessAllWithError)
{
    ...
    auto process_func = [](const std::string&, const std::string&) -> bool
    {
        return false; // Всегда возвращаем ошибку
    };
